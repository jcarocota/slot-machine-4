...
export class PaylineGraphic extends PIXI.Container {
    ...
    constructor(paylinePoints: boolean[][], coincidences: number, slotWidth=0, slotHeight=0, reelsWindowX=0, reelsWindowY = 0) {
        super();
       ...
       for (let columnID = 0; columnID < numColumns; columnID++) {
           for (let rowID = 0; rowID < numRows; rowID++) {
               if(this.paylinePoints[rowID][columnID]) {
                   let pointX = this.reelsWindowX + (this.slotWidth/2) + (this.slotWidth*columnID);
                   let pointY = this.reelsWindowY + (this.slotHeight/2) + (this.slotHeight*rowID);
